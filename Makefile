...
.PHONY: help setup init plan apply apply-auto destroy output fmt validate lint-init lint ruff-check ruff-format ruff-fix check clean deploy update-schemas test-lambdas test-setup test-token test-evals test-ground-truth test-all-evals
...
test-setup: ## One-time setup: write test config to .env and get JWT token
	@echo "Setting up test configuration..."
	@# Get GATEWAY_ID from terraform
	$(eval GATEWAY_ID := $(shell $(TF) output -raw gateway_id 2>/dev/null))
	@if [ -z "$(GATEWAY_ID)" ]; then \
		echo "Error: gateway_id not found. Run 'make apply' first."; \
		exit 1; \
	fi
	@# Remove old test config section if present
	@sed -i '' '/^# --- Test Configuration/,/^$$/d' $(TF_DIR)/config/.env 2>/dev/null || true
	@# Append test config
	@echo "" >> $(TF_DIR)/config/.env
	@echo "# --- Test Configuration (generated by make test-setup) ---" >> $(TF_DIR)/config/.env
	@echo "GATEWAY_ID=$(GATEWAY_ID)" >> $(TF_DIR)/config/.env
	@echo "GROUND_TRUTH_PROFILE=$(GROUND_TRUTH_PROFILE)" >> $(TF_DIR)/config/.env
	@# Prompt for OAuth credentials if not already set
	@if grep -q "^GATEWAY_CLIENT_ID=" $(TF_DIR)/config/.env 2>/dev/null; then \
		echo "OAuth credentials already in .env"; \
	else \
		echo ""; \
		echo "Gateway OAuth Configuration (from your IDP):"; \
		read -p "  Client ID: " client_id; \
		read -p "  Client Secret: " client_secret; \
		read -p "  Token URL: " token_url; \
		read -p "  Authorize URL: " authorize_url; \
		echo "GATEWAY_CLIENT_ID=$$client_id" >> $(TF_DIR)/config/.env; \
		echo "GATEWAY_CLIENT_SECRET=$$client_secret" >> $(TF_DIR)/config/.env; \
		echo "GATEWAY_TOKEN_URL=$$token_url" >> $(TF_DIR)/config/.env; \
		echo "GATEWAY_AUTHORIZE_URL=$$authorize_url" >> $(TF_DIR)/config/.env; \
	fi
	@echo ""
	@echo "Test config written to $(TF_DIR)/config/.env"
	@echo "Run 'make test-token' to authenticate and cache a JWT token."

test-token: ## Get a JWT token for Gateway tests (opens browser)
	...
	uv run --group test python tests/helpers/oauth_token.py
